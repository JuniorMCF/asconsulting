<template>
    <v-container class="content-wrap-2 pa-0 mx-auto">
                ...
                <v-sheet elevation="0" class="py-4 px-1">
                    <v-chip-group show-arrows active-class="primary--text">
                        <v-chip
                            ...
                            v-for="(categoria, i) in categorias" :key="i" @click.prevent="goToPublicaciones(categoria)">
            ...
            <v-col class="col-12 ma-0 pa-4 px-md-16 d-flex flex-wrap align-center">
                <v-card-text class="content-wrap-0 mx-auto">
                    <div style="height: 50px">
                        <lottie-animation ref="anim" :loop="true" :autoPlay="true" :loopDelayMin="2.5" :loopDelayMax="5"
                            :speed="1" :width="50" :height="50" :animationData="require('/lottie/loading.json')" />
                    </div>
                </v-card-text>
            </v-col>
        </v-row>
                    ...
                    <v-list-item-avatar color="grey">
                        ...
                        <v-avatar v-else class="grey lighten-3 pa-1">
                            <svg version="1.1" xmlns="http://www.w3.org/2000/svg"
                                ...
                                enable-background="new 0 0 60 60;" xml:space="preserve" height="36" width="36"
                                class="s2FZSQ">
                                <g>
                                    <ellipse fill="#979797" cx="30.7" cy="22.3" rx="11.5" ry="11.6" />
                                    <g>
                                        <ellipse fill="none" cx="30.7" cy="22.3" rx="11.5" ry="11.6" />
                                        <path fill="#979797" d="M29.9,37.1c-9,0-17,3.9-21.4,10.1c-0.5,0.7-1,1.4-1.4,2.1c0.5,0.7,1.1,1.3,1.7,1.9c1,1,2.1,2,3.3,2.8
                    ...
                    <v-list-item-action>
                        <v-menu bottom left>
                            <template v-slot:activator="{ on, attrs }">
                                <v-btn dark icon v-bind="attrs" v-on="on" text class="elevation-0 primary--text" fab
                                    small>
                            ...
                            <v-list class="pa-0 ma-0">
                                <v-list-item link>
                                    <v-list-item-title class="caption" @click.prevent="showSharedDialog(post)">
        ...
        <v-row class="pa-0 ma-0 content-wrap-article mx-auto elevation-0" v-if="!loadingFilterPosts">
            <v-col class="col-12 ma-0 pa-4 px-md-16 pb-md-16" style="min-height: 588px">
                <v-card-text class="pa-0">
                    ...
                    <div v-if="post.file" style="position: relative;overflow: hidden;width: 100%;">
                        <embed :src="post.file" style="height:500px; width: 100%;" />
                    </div>
                </v-card-text>
                    ...
                    <v-chip-group column>
                        <div v-for="tag in tags" :key="tag.id">
                            <v-hover v-slot="{ hover }">
                                <v-chip @click="goToTag(tag)" color="black" outlined
                                    class="black elevation-0 px-6 py-4 rounded-0 subtitle-2"
                                    :class="{ 'grey lighten-2 ': hover }">{{ tag.tag }}</v-chip>
                            </v-hover>
                        </div>
                    </v-chip-group>
                </v-card-text>
        ...
        <v-row class="pa-0 ma-0 content-wrap-article mx-auto elevation-0" v-if="!loadingFilterPosts">
            ...
            <v-col class="col-12 ma-0 pa-2 d-flex flex-wrap space-between align-center pa-4 px-md-16">
                <ShareNetwork network="facebook" :url="getSharedPost(post.route)" :title="post.titulo"
                    :description="post.contenido">
                    <v-btn fab small class="elevation-0 mr-2">
                        <v-icon color="primary">mdi-facebook</v-icon>
                    </v-btn>
                </ShareNetwork>
                <ShareNetwork network="twitter" :url="getSharedPost(post.route)" :title="post.titulo"
                    :description="post.contenido">
                ...
                <ShareNetwork network="linkedin" :url="getSharedPost(post.route)" :title="post.titulo"
                    :description="post.contenido">
                    <v-btn fab small class="elevation-0 mx-2">
                ...
                <v-spacer></v-spacer>
                <v-btn text class="caption" color="primary" v-for="categoria in post.categorias"
                    :key="'categoria-' + categoria.id" @click.prevent="goToCategorys(categoria)">{{
        ...
        <v-row class="pa-0 ma-0 content-wrap-article mx-auto elevation-0" v-if="!loadingFilterPosts">
            <v-col class="col-12 pa-2 d-flex flex-wrap space-between align-center pa-4 px-md-16 pb-md-16">
            ...
            <v-col class="col-12 d-flex flex-wrap pa-4 px-md-16">
                <v-avatar class="grey lighten-3 pa-1">
                    <svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        ...
                        height="36" width="36" class="s2FZSQ">
                        <g>
                            <ellipse fill="#979797" cx="30.7" cy="22.3" rx="11.5" ry="11.6" />
                            <g>
                                <ellipse fill="none" cx="30.7" cy="22.3" rx="11.5" ry="11.6" />
                ...
                <div style="width: calc(100% - 50px) !important" class="align-center">
                    <v-text-field class="ml-4 my-0 py-2" hide-details="auto" placeholder="Escribir un comentario"
                        v-model="comment" @focus="isComment = true" maxlength="500" counter></v-text-field>
                    <v-card-text class="py-0 d-flex flex-wrap space-between" v-if="isComment">
                        <div>
                            <v-btn fab small text class="elevation-0" @click.prevent="showEmojis = !showEmojis">
                                <v-icon>mdi-emoticon-outline</v-icon>
                            </v-btn>
                            <VEmojiPicker @select="selectEmoji" v-if="showEmojis" class="showEmoji" />
                        </div>
                        <v-spacer></v-spacer>
                        <v-btn text color="primary" class="text-normal elevation-0 rounded-0"
                            @click.prevent="cancelComment()">Cancelar</v-btn>
                        <v-btn color="primary" class="text-normal elevation-0 rounded-0 ml-2"
                            @click.prevent="sendComment()">Publicar</v-btn>
                    </v-card-text>
                </div>
            </v-col>
        </v-row>
        <v-row class="pa-0 ma-0 content-wrap-article mx-auto elevation-0" v-if="!loadingFilterPosts">
            <v-col class="col-12 d-flex flex-wrap pa-4 px-md-16" v-for="comment in post.comments"
            ...
            M30.7,33.9c-6.4,0-11.5-5.2-11.5-11.6c0-6.4,5.2-11.6,11.5-11.6s11.5,5.2,11.5,11.6C42.3,28.7,37.1,33.9,30.7,33.9z" />
                ...
                <div style="width: calc(100% - 50px) !important">
                    <v-card solo auto-grow filled disabled class="elevation-0 caption black--text ml-4" color="white"
                        rows="1" max-height="200">
