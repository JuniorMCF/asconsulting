<template>
  <div>
    <v-card-title class="px-10">
      {{ actualPage }} <v-spacer></v-spacer>
      <v-btn class="sky white--text text-normal rounded-xl ekevation-1"
        ><v-icon left>mdi-plus</v-icon> Crear nueva entrada</v-btn
      >
    </v-card-title>
    <v-container fluid class="pa-0 ma-0 px-10">
      <v-row class="pa-0 ma-0">
        <v-col class="pa-0 ma-0">
          <v-card-text class="px-0">
            <span class="title font-weight-bold subtitle-1"
              >Haz un seguimiento de tus últimas entradas</span
            >
          </v-card-text>
        </v-col>
      </v-row>
            ...
            <template v-slot:top>
              <v-card-text flat class="px-0 d-flex flex-wrap">
                <v-toolbar-title class="px-0 subtitle-1 font-weight-bold"
                  >Últimas publicaciones</v-toolbar-title
                >
                <v-divider class="mx-4" inset vertical></v-divider>
                <v-spacer></v-spacer>

                <v-btn text color="sky" class="text-normal caption">
                  <v-icon color="sky" left small> mdi-chart-bar </v-icon>
